import { pgTable, text, integer, boolean, timestamp, uuid, index, uniqueIndex, foreignKey, numeric, primaryKey } from "drizzle-orm/pg-core";
...
export const taskComments = pgTable("task_comments", {
    id: text("id").primaryKey().notNull(), // Assuming string ID for compatibility or external system
    taskId: uuid("task_id").notNull().references(() => tasks.id, { onDelete: "cascade" }),
    ...
    createdAt: timestamp("created_at").defaultNow().notNull(), // text in source, fixed to timestamp
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (table) => ({
...
export const taskDocumentLinks = pgTable("task_document_links", {
    id: text("id").primaryKey().notNull(), // Assuming string ID
    taskId: uuid("task_id").notNull().references(() => tasks.id, { onDelete: "cascade" }),
