...
export function IssueRow({
  ...
  return (
    <div
      className={cn(
        'group flex items-center gap-3 px-4 py-2.5 hover:bg-muted/50 cursor-pointer border-b border-border/30 last:border-b-0',
        selected && 'bg-muted ring-2 ring-indigo-500 ring-inset'
      )}
      onClick={onClick}
    >
      {/* Priority indicator */}
          ...
          {issues.map((issue) => {
            ...
            return (
              <div
                key={issue.id}
                className={cn(
                  'group bg-background border border-border rounded-lg p-3 hover:border-border/80 cursor-pointer shadow-sm',
                  selectedIssueId === issue.id && 'ring-2 ring-indigo-500'
                )}
                onClick={() => onIssueClick?.(issue)}
              >
                {/* Card header */}
