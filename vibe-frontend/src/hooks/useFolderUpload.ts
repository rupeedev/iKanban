...
async function uploadSingleFile(
  ...
  for (let attempt = 0; attempt <= UPLOAD_CONFIG.maxRetries; attempt++) {
    try {
      ...
      const { url, storage_provider, file_path } =
        await documentsApi.getUploadUrl(teamId, file.name, currentFolderId);
  ...
  throw (
    lastError ||
    new Error(
      `Failed to upload ${file.name} after ${UPLOAD_CONFIG.maxRetries} retries`
    )
  );
}
...
export function useFolderUpload(
  ...
  const uploadFolder = useCallback(
    async (files: FileList) => {
      ...
      for (
        let batchStart = 0;
        batchStart < fileArray.length;
        batchStart += UPLOAD_CONFIG.batchSize
      ) {
        const batch = fileArray.slice(
          batchStart,
          batchStart + UPLOAD_CONFIG.batchSize
        );
        ...
        setProgress((prev) =>
          prev ? { ...prev, uploaded: uploadedCount, errors } : null
        );
